...
class TransactionBuilder {
    ...
    build() {
        ...
        // todo: we sign only inputs here. In Bitcoin we sign a full copy of a transaction (including all inputs and outputs).
        // todo: in lhartikk navivechain signature for an input is created for TXID (which includs all inputs and outputs). See https://lhartikk.github.io/jekyll/update/2017/07/12/chapter3.html
        ...
        let inputs = R.map((utxo) => {
            ...
            // todo: every input should be signed by its own secret key.
            utxo.signature = CryptoEdDSAUtil.signHash(self.secretKey, txiHash);
            return utxo;
        }, this.listOfUTXO);
