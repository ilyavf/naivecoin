...
class Transaction {
    ...
    check() {
        ...
        if (!isTransactionHashValid) {
            ...
            throw new TransactionAssertionError(`Invalid transaction hash '${this.hash}'`, null, this);
        }
        ...
        R.map((txInput) => {
            ...
            if (!isValidSignature) {
                ...
                throw new TransactionAssertionError(`Invalid transaction input signature '${JSON.stringify(txInput)}'`, null, txInput);
            }
        }, this.data.inputs);
        ...
        if (this.type == 'regular') {
            ...
            if (!isInputsAmountGreaterOrEqualThanOutputsAmount) {
                ...
                throw new TransactionAssertionError(`Invalid transaction balance: inputs sum '${sumOfInputsAmount}', outputs sum '${sumOfOutputsAmount}'`, null, { sumOfInputsAmount, sumOfOutputsAmount });
            }
            ...
            if (!isEnoughFee) {
                ...
                throw new TransactionAssertionError(`Not enough fee: expected '${Config.FEE_PER_TRANSACTION}' got '${(sumOfInputsAmount - sumOfOutputsAmount)}'`, null, { sumOfInputsAmount, sumOfOutputsAmount, FEE_PER_TRANSACTION: Config.FEE_PER_TRANSACTION });
            }
        }
    ...
    // todo: to use tx-builder we can override this method. It is called by Block.fromJson
